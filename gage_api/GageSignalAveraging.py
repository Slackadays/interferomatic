...
def initialize_signalAveraging(handle):
    expert_options = gc.CS_BBOPTIONS_MULREC_AVG_TD
    acq = PyGage.GetAcquisitionConfig(handle)
    if not isinstance(acq, dict):
        if not acq:
            print("Error in call to GetAcquisitionConfig")
            return ge.CS_MISC_ERROR
    ...
    if extended_options & expert_options:
        print("\nSelecting Expert Multiple Record Average from image 1")
        acq['Mode'] |= gc.CS_MODE_USER1
    elif (extended_options >> 32) & expert_options:
        print("\nSelecting Expert Multiple Record Average from image 2")
        acq['Mode'] |= gc.CS_MODE_USER2
    else:
        print("\nCurrent system does not support Expert Multiple Record Average")
...
#        handle = PyGage.GetSystem(gc.CSPCIEx_BOARDTYPE | gc.CS16x2_BOARDTYPE, 0, 0, 0)
...
def save_data_to_file(handle, mode, app, system_info):
    ...
    # The Transfer is done with TxMode set to gc.TxMODE_DATA_32 because the data for each average is added
    # together so it can theoretically end up being a 32 bit value. To get the actual average you need
    # to divide by the number of averages.

    count = 0
    for i in range(1, system_info['ChannelCount'] + 1, channel_increment):
        for group in range(app['SegmentStart'], app['SegmentStart'] + app['SegmentCount']):
            buffer = PyGage.TransferData(handle, i, gc.TxMODE_DEFAULT | gc.TxMODE_DATA_32, group, app['StartPosition'], app['TransferLength'])
            ...
            stHeader['TimeStamp'] = 0 # no timestamp in signal averaging
...
def main():
    ...
    try:
        ...
        status = initialize_signalAveraging(handle)
        if (status < 0):
            error_string = PyGage.GetErrorString (status)
            print("Error: " , error_string)
            PyGage.FreeSystem(handle)
        ...
        PyGage.SetMulRecAverageCount(handle, 500) # must do before the commit
        ...
        if (status < 0):
            ...
            # signal averaging only supports saving files as voltages in float format
            if app["SaveFileFormat"] != gs.TYPE_FLOAT:
                app["SaveFileFormat"] = gs.TYPE_FLOAT
                print("\nThis program only supports saving data as voltages in float format")
                print("The file format will be changed to TYPE_FLOAT\n")
