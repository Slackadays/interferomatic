...
def save_data_to_file(handle, mode, app, system_info):
    ...
    for i in range(1, system_info['ChannelCount'] + 1, channel_increment):
        # We're adding 64 samples padding in case the driver adjusts the transfer length
        buffer = PyGage.TransferData(handle, i, 0, 1, app['StartPosition'], app['TransferLength'] + 64)
        ...
        start_offset = app['StartPosition'] - buffer[2]
        if start_offset < 0:
            start_offset = 0

        length = buffer[2] - start_offset
        # Save the smaller of the requested transfer length and the actual transfer length
        if app['TransferLength'] <= length:
            saved_length = app['TransferLength']
        else:
            saved_length = length

        stHeader['Length'] = saved_length
